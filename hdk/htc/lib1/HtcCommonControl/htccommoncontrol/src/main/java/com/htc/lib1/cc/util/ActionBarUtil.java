...
public class ActionBarUtil {
     ...
     *            automotive mode
     * @hide
     * @deprecated use {@link #getItemWidth(Context, int)} instead.
     **/
    public static int getItemWidth(Context context, boolean isAutomotive) {
        return getItemWidth(context, ActionBarItemView.MODE_THEME);
    }
     ...
     *
     * @param context the relative Context that has the Resources
     * @param mode should be one of {@link ActionBarItemView#MODE_THEME},
     *            {@link ActionBarItemView#MODE_EXTERNAL},{@link ActionBarItemView#MODE_AUTOMOTIVE}.
     * @hide
     **/
    public static int getItemWidth(Context context, int mode) {
        if (null == context) return 0;

        Resources r = context.getResources();
        if (null == r) return 0;

        final int baseWidth = Math.min(r.getDisplayMetrics().widthPixels, r.getDisplayMetrics().heightPixels);
        float fractionWidth = r.getFraction(R.fraction.ab_item_width_percent, baseWidth, 1);

        final int defStyleAttr = mode == ActionBarItemView.MODE_THEME ? R.attr.htcActionButtonStyle : 0;
        final int defStyleRes = ActionBarItemView.getDefStyleRes(mode);

        TypedArray a = context.obtainStyledAttributes(null, R.styleable.HtcActionButton, defStyleAttr, defStyleRes);
        fractionWidth = a.getFraction(R.styleable.HtcActionButton_android_keyWidth, baseWidth, 1, fractionWidth);
        a.recycle();

        return isOdd((int) fractionWidth);
    }
    ...
    public static Drawable getActionBarBackground(Context context, int mode) {
        ...
        if (mode == ActionBarExt.MODE_TRANSPARENT) {
            ...
            d = context.getResources().getDrawable(R.drawable.common_app_bkg_top_full);
        } else if (mode == ActionBarExt.MODE_GRADIENT_TRANSPARENT) {
            // fullscreen is true backgound
            d = context.getResources().getDrawable(R.drawable.common_app_bkg_top_full_land);
        } else {
            // theme color
            TypedValue outValue = new TypedValue();
            context.getTheme().resolveAttribute(R.attr.multiply_color, outValue, true);
            d = context.getResources().getDrawable(outValue.resourceId);
        }
        return d;
    }
    ...
    public static void setActionModeBackground(ContextThemeWrapper context, ActionMode actionMode, Drawable backgroundDrawable) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && actionMode.getType() == ActionMode.TYPE_FLOATING) {
            if (HtcBuildFlag.Htc_DEBUG_flag) {
                Log.e("HTCActionBar", "actionMode type is TYPE_FLOATING", new Exception());
            }
            return;
        }
