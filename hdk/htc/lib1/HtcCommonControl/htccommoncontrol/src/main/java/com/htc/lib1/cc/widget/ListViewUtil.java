...
class ListViewUtil {
    ...
    void init(AbsListView absListView, Callbacks callbacks) {
        ...
        if(sListItemHeight <= 0 ) {
            sListItemHeight = HtcListItemManager.getInstance(context).getDesiredListItemHeight(HtcListItem.MODE_DEFAULT);
        }
        PULL_DOWN_REFRESH_GAP = sListItemHeight;
        MAX_OVERSCROLL_GAP = 2 * PULL_DOWN_REFRESH_GAP;
        boolean isAutomotiveMode = HtcCommonUtil.getThemeType(context) == HtcCommonUtil.THEME_CAR;
        TypedArray a = context.obtainStyledAttributes(null, R.styleable.HtcListView, android.R.attr.listViewStyle, R.style.HtcListView);
        mDividerMargin = res.getDimensionPixelOffset(a.getResourceId(R.styleable.HtcListView_android_insetLeft, R.dimen.common_list_divider_margin));
        mSectionDividerMarginLeft = isAutomotiveMode ? HtcListItemManager.getActionButtonWidth(context, HtcListItem.MODE_AUTOMOTIVE) : HtcListItemManager.getInstance(context).getPhotoFrameWidth(context, HtcListItem.MODE_DEFAULT);
        mSectionDivider = res.getDrawable(a.getResourceId(R.styleable.HtcListView_android_horizontalDivider, R.drawable.common_list_divider));
        a.recycle();
        ...
        if (mListView instanceof ListView) {
            ...
            if (childIndex >= 0) {
                ...
                if (mDividerController != null && dividerType == IDividerController.DIVIDER_TYPE_SECTION) {
                    ...
                    bounds.left += mSectionDividerMarginLeft;
