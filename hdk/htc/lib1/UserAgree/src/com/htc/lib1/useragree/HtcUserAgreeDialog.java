...
public class HtcUserAgreeDialog {
	...
	/**
	 * This is to launch a HtcUserAgreeDialog with async writing preference flag.
	 *
	 ...
	 * @param isAsync async writing preference flag
	 */
	public static void launchUserAgreeDialog(final Context context, final OnUserClickListener onUserClickListener, UserAgreeContent content, boolean isAsync) {
		if (DEBUG) Log.d(TAG, "User agree dialog with async flag is launched");

		launchUserAgreeDialog(context, onUserClickListener, content, null, -1, isAsync);
	}
	...
	public static void launchUserAgreeDialog(final Context context, final OnUserClickListener onUserClickListener, UserAgreeContent content) {
		...
		launchUserAgreeDialog(context, onUserClickListener, content, null, -1, false);
	}
	...
	public static void launchUserAgreeDialogFromIME(final Context context, final OnUserClickListener onUserClickListener, UserAgreeContent content, View inputView) {
		...
		launchUserAgreeDialog(context, onUserClickListener, content, inputView, android.view.WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED, false);
	}
	...
	public static void launchUserAgreeDialogWithWindowsFlag(final Context context, final OnUserClickListener onUserClickListener, UserAgreeContent content, int flag) {
		...
		launchUserAgreeDialog(context, onUserClickListener, content, null, flag, false);
	}
	...
	private static void launchUserAgreeDialog(final Context context, final OnUserClickListener onUserClickListener, UserAgreeContent content, View inputView, int flag, final boolean isAsync){
		...
		if (needShowUserAgreeDialog(context)) {
			...
			builder.setCheckBox(checkboxLabel, false, new CompoundButton.OnCheckedChangeListener() {
			...
			builder.setPositiveButton(positiveLabel, new OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					if (bCheckboxSelected) {
						saveShowUserAgreeDialog(context, false, isAsync);
					}
	...
	private static void saveShowUserAgreeDialog(Context mContext, boolean set, boolean isAsync) {
