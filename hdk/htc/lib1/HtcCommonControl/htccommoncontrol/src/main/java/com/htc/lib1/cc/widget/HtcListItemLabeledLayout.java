...
public class HtcListItemLabeledLayout extends ViewGroup implements IHtcListItemTextComponent,
    ...
    private void setDefaultTextStyle(AttributeSet attrs) {
        TypedArray a = HtcListItemManager.obtainStyleIdSet(mItemMode,
                HtcListItem2TextComponent.MODE_UNKNOWN_LIST, 0, getContext(), attrs);
        int descriptionTextStyleId = a.getResourceId(R.styleable.HtcListItemAppearance_android_textAppearanceLargeInverse,
                R.style.list_body_primary_m);
        int otherTextStyleId = a.getResourceId(R.styleable.HtcListItemAppearance_android_textAppearanceSmallInverse,
                R.style.separator_primary_l);
        a.recycle();
        if (mDescriptionText)
            mLabelView.setTextStyle(descriptionTextStyleId);
        else
            mLabelView.setTextStyle(otherTextStyleId);
    }
