...
public class HtcOverlapLayout extends ViewGroup {
    ...
    protected boolean fitSystemWindows(Rect insets) {
        ...
        /* fitSystemWindows was deprecated in API level 20*/
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT_WATCH) {
            fitSystemWindow(insets);
        }
    ...
    public WindowInsets onApplyWindowInsets(WindowInsets insets) {
        fitSystemWindow(new Rect(insets.getSystemWindowInsetLeft(), insets.getSystemWindowInsetTop(), insets.getSystemWindowInsetRight(), insets.getSystemWindowInsetBottom()));
        return super.onApplyWindowInsets(insets);
    }

    private void fitSystemWindow(Rect insets){
        if (DEBUG)
