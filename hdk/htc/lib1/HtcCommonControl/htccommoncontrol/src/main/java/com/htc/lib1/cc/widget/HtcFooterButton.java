...
public class HtcFooterButton extends TextView {
    ...
    private int mRightWithImageTextStyleId = 0;
    @ExportedProperty(category = "CommonControl", resolveId = true)
    private int mRightWithoutImageTextStyleId = 0;
    @ExportedProperty(category = "CommonControl", resolveId = true)
    private int mBottomWithImageTextStyleId = 0;
    @ExportedProperty(category = "CommonControl", resolveId = true)
    private int mBottomWithoutImageTextStyleId = 0;
    ...
    public HtcFooterButton(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.HtcFooterButton,
                defStyle, R.style.FooterBarButtonStyle);
        ...
        mRightWithImageTextStyleId = a.getResourceId(R.styleable.HtcFooterButton_android_textAppearanceSmall, R.style.fixed_label_off_m);
        mRightWithoutImageTextStyleId = a.getResourceId(R.styleable.HtcFooterButton_android_textAppearanceSmallInverse, R.style.fixed_separator_secondary_xs);
        mBottomWithImageTextStyleId = a.getResourceId(R.styleable.HtcFooterButton_android_textAppearanceLarge, R.style.fixed_label_off_m);
        mBottomWithoutImageTextStyleId = a.getResourceId(R.styleable.HtcFooterButton_android_textAppearanceLargeInverse, R.style.fixed_separator_secondary_m);

        mDisplayMode = a.getInt(R.styleable.HtcFooterButton_footer_display_mode, HtcFooter.DISPLAY_MODE_DEFAULT);
        mStyleMode = a.getInt(R.styleable.HtcFooterButton_backgroundMode, HtcFooter.STYLE_MODE_DEFAULT);

        isEmpty = isTextEmpty();
        if(mStyleMode == HtcFooter.STYLE_MODE_LIGHT) {
            setBackgroundResource(a.getResourceId(R.styleable.HtcFooterButton_android_listSelector, R.drawable.list_selector_light));
        }else{
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.save();
        canvas.translate((mCanvasRect.left + mCanvasRect.right - dWidth) * 0.5f, yOffset);
        mImage.draw(canvas);
        canvas.restore();
        safeDraw(canvas, mCanvasRect);
    }
    ...
    private void setRightTextAppearance() {
        if (mStyleMode == HtcFooter.STYLE_MODE_LIGHT) {
            if (mImage != null)
                setTextAppearance(getContext(), mRightWithImageTextStyleId == 0 ? R.style.fixed_label_off_m : mRightWithImageTextStyleId);
            else
                setTextAppearance(getContext(), mRightWithoutImageTextStyleId == 0 ? R.style.fixed_separator_secondary_xs : mRightWithoutImageTextStyleId);
        } else {
            if (mImage != null)
    ...
    private void setBottomTextAppearance() {
        if (mStyleMode == HtcFooter.STYLE_MODE_LIGHT) {
            if (mImage != null)
                setTextAppearance(getContext(), mBottomWithImageTextStyleId == 0 ? R.style.fixed_label_off_m : mBottomWithImageTextStyleId);
            else
                setTextAppearance(getContext(), mBottomWithoutImageTextStyleId == 0 ? R.style.fixed_separator_secondary_m : mBottomWithoutImageTextStyleId);
        } else {
            if (mImage != null)
                setTextAppearance(getContext(), R.style.fixed_label_on_m);
            else
