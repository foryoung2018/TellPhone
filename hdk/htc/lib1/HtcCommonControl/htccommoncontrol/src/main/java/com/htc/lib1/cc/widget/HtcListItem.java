...
public class HtcListItem extends FrameLayout {
    ...
    private HtcListItem(Context context, AttributeSet attrs, int defStyle,int mode) {
        ...
        initDrawablesAndItemMode(context, attrs, mode);
        initHeights();
        initListener();
    }
    ...
    private void onChildAdded(View child) {
        ...
        if (mMode == MODE_KEEP_MEDIUM_HEIGHT || mMode == MODE_POPUPMENU) {
            ...
            if (child instanceof IHtcListItemTextComponent) {
                ...
                if (i == 0) {
                    if (mFirstComponentAlign) {
                        ...
                        usedWidth -= mListItemStartMargin;
                        usedWidth -= mListItemMargin;
                        usedWidth += ((IHtcListItemComponentNoLeftTopMargin) child).getRightMargin();
                    } else {
                        // margin M2
                        usedWidth += child.getMeasuredWidth();
                    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (firtChild == null || firtChild.getVisibility() == View.GONE) {
            firstChildType = FIRST_CHILD_NORMAL;
        } else if (mFirstComponentAlign) {
            firstChildType = FIRST_CHILD_COMPONENT_ALIGN;
        } else if (firtChild instanceof IHtcListItemComponentNoLeftTopMargin) {
            firstChildType = FIRST_CHILD_NO_MARGINS;
        } else {
            firstChildType = FIRST_CHILD_NORMAL;
        }
        ...
        if (firstChildType == FIRST_CHILD_COMPONENT_ALIGN) {
        ...
        } else if (firstChildType == FIRST_CHILD_NO_MARGINS) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (firstChildType == FIRST_CHILD_COMPONENT_ALIGN && i == 0) {
            ...
            } else if (firstChildType == FIRST_CHILD_NO_MARGINS && i == 0) {
                left += ((IHtcListItemComponentNoLeftTopMargin) child).getRightMargin();
            } else {
                left += mListItemMargin;
            }
            lastChild = child;
