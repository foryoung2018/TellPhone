...
public class HtcEmptyView extends ViewGroup {
    ...
    public HtcEmptyView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.htcEmptyViewStyle);
    }
    ...
    public HtcEmptyView(Context context, AttributeSet attrs, int defStyle) {
        ...
        setText(mEmptyString);
    }

    private void init(AttributeSet attrs, int defStyleAttr, boolean loadAll) {
        final int defStyleAttrId = mMode == HtcButtonUtil.BACKGROUND_MODE_THEME ? defStyleAttr : 0;
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.HtcEmptyView, defStyleAttrId, getDefStyleRes());
        if (loadAll) mEmptyString = a.getString(R.styleable.HtcEmptyView_android_text);
        mEmptyStringStyle = a.getResourceId(R.styleable.HtcEmptyView_android_titleTextStyle, R.style.list_body_secondary_l);
        mPullStringStyle = a.getResourceId(R.styleable.HtcEmptyView_android_subtitleTextStyle, R.style.fixed_list_secondary);
        a.recycle();
    }
    ...
    private int getDefStyleRes() {
        switch (mMode) {
            case MODE_NORMAL:
                return R.style.HtcEmptyView_Light;
            case MODE_DARK:
                return R.style.HtcEmptyView;
            default:
                return R.style.HtcEmptyView_Light;
        }
    }
    ...
    public void setMode(int mode) {
        if (mode == mMode || !isValidMode(mode)) {
            return;
        }
        mMode = mode;
        init(null, R.attr.htcEmptyViewStyle, false);
        mModeChanged = true;
    }
    ...
    private boolean isValidMode(int mode) {
        if (mode == HtcButtonUtil.BACKGROUND_MODE_THEME || mode == MODE_NORMAL || mode == MODE_DARK) {
            return true;
        } else {
            return false;
        }
