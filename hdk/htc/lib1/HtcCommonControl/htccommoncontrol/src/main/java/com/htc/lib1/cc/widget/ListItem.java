...
public class ListItem extends ViewGroup {
    ...
    private int makeChildHeightMeasureSpec(int spec, LayoutParams lp) {
        final int padding = getPaddingTop() + getPaddingBottom() + lp.topMargin + lp.bottomMargin;
    ...
    private void measureRestFixedSizeChild(int count, int widthSize, int heightMeasureSpec) {
        for (int i = 0; i < count; i++) {
            ...
            if (checkType(type, LayoutParams.TYPE_SIZE_REST_SET)) {
                ...
                child.measure(MeasureSpec.makeMeasureSpec(restSize, MeasureSpec.EXACTLY), makeChildHeightMeasureSpec(heightMeasureSpec, lp));
            }
        }
    }
    ...
    private void measureRestAutoSizeChild(int count, int widthSize, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (checkType(type, LayoutParams.TYPE_SIZE_AUTO_REST_SET)) {
                child.measure(MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED), makeChildHeightMeasureSpec(heightMeasureSpec, lp));
        ...
        for (int i = 0; i < count; i++) {
            ...
            final int type = lp.type;
            if (checkType(type, LayoutParams.SIZE_AUTO_1_3_REST)) {
                child.measure(MeasureSpec.makeMeasureSpec(restSize_1_3_Final, MeasureSpec.EXACTLY), makeChildHeightMeasureSpec(heightMeasureSpec, lp));
            } else if (checkType(type, LayoutParams.SIZE_AUTO_2_3_REST)) {
                child.measure(MeasureSpec.makeMeasureSpec(restSize_2_3_Final, MeasureSpec.EXACTLY), makeChildHeightMeasureSpec(heightMeasureSpec, lp));
            }
        }
    }
        ...
        if (checkType(type, LayoutParams.TYPE_SIZE_FIX_WRAP_CONTENT_SET)) {
            ...
            if (checkType(type, LayoutParams.SIZE_147)) {
                child.measure(MeasureSpec.makeMeasureSpec(mSize147,
                        MeasureSpec.EXACTLY), makeChildHeightMeasureSpec(heightMeasureSpec, lp));
                ...
                child.measure(MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED),
                        makeChildHeightMeasureSpec(heightMeasureSpec, lp));
