...
public class HtcListItem1LineCenteredText extends FrameLayout implements IHtcListItemTextComponent,
    ...
    private void setDefaultTextStyle(AttributeSet attrs) {
        int preferStyleId = mIsNoContentText ? (HtcListItemManager.checkWhiteTextMode(getContext(), mMode) ? R.style.htcListItem_NoContent : R.style.htcListItem_Dark_NoContent) : 0;
        TypedArray a = HtcListItemManager.obtainStyleIdSet(mItemMode, mMode, preferStyleId, getContext(), attrs);
    ...
    public void setView(View v) {
        if (v != null && mImage == null) {
            ...
            addView(mImage, new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
                    ViewGroup.LayoutParams.WRAP_CONTENT));
            TypedArray a = HtcListItemManager.obtainStyleIdSet(mItemMode, mMode, 0, getContext(), null);
            int styleId = a.getResourceId(R.styleable.HtcListItemAppearance_android_textColorPrimaryDisableOnly,
                    R.style.fixed_darklist_primary_xxs);
            a.recycle();
            setTextStyle(styleId);
        } else if (v == null) {
            removeView(mImage);
