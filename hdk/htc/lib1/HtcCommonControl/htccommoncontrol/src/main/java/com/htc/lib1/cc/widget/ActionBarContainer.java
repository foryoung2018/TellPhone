...
public class ActionBarContainer extends ViewGroup {
    ...
    private void setupEnvironment() {
        final int defStyleAttr = mSupportMode == HtcButtonUtil.BACKGROUND_MODE_THEME ? R.attr.htcActionBarStyle : 0;
        TypedArray a = getContext().obtainStyledAttributes(null, R.styleable.HtcActionBar, defStyleAttr, getDefStyleRes());
        mCommonOffset = a.getDimensionPixelOffset(R.styleable.HtcActionBar_android_progressBarPadding, mMeasureSpecM2);
        mBackUpDrawable = a.getDrawable(R.styleable.HtcActionBar_android_homeAsUpIndicator);
        mBackUpM2Flag = a.getBoolean(R.styleable.HtcActionBar_android_autoStart, true);
        setBackground(a.getDrawable(R.styleable.HtcActionBar_android_background));
        a.recycle();
        if (mBackUpDrawable == null) {
            mBackUpDrawable = getContext().getResources().getDrawable(R.drawable.icon_btn_up_dark_xl);
        }
        mContainerHeight = ActionBarUtil.getActionBarHeight(getContext(), false);
    }
    ...
    private void setBackUpDrawable() {
        ...
        int mode = convertMode_ABC2ABI(mSupportMode);
        if (mBackUpM2Flag) mode |= ActionBarItemView.FLAG_M2_IMG_M2;
        mBackupView.setSupportMode(mode);

        mBackupView.setIcon(mBackUpDrawable);

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            mBackUpDrawable.setAutoMirrored(true);
        }
    }

    private int convertMode_ABC2ABI(int mode) {
        switch (mode) {
            case MODE_EXTERNAL:
                return ActionBarItemView.MODE_EXTERNAL;
            case MODE_AUTOMOTIVE:
                return ActionBarItemView.MODE_AUTOMOTIVE;
            default:
                return ActionBarItemView.MODE_THEME;
        }
    }
    ...
    private boolean isValidMode(int mode) {
        if (mode == MODE_EXTERNAL || mode == HtcButtonUtil.BACKGROUND_MODE_THEME) {
            return true;
        } else {
            if (ENABLE_DEBUG) android.util.Log.d("HTCActionBar", "Invalid mode:" + mode);
            return false;
        }
    }
