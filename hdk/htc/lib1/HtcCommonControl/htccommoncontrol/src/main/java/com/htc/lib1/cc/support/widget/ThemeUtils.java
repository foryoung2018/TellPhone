...
/**
 * Utility class to help resolve the color attr in current theme and get the specified state color if it is a ColorStateList.
 */
class ThemeUtils {
    ...
    static int getDisabledColor(Context context, int colorId, float disabledAlpha) {
        ColorStateList csl = ContextCompat.getColorStateList(context, colorId);
        if (csl.isStateful()) {
            // If the CSL is stateful, we'll assume it has a disabled state and use it
            return csl.getColorForState(DISABLED_STATE_SET, csl.getDefaultColor());
        } else {
            ...
            final int originalAlpha = Color.alpha(color);
            return ColorUtils.setAlphaComponent(color, Math.round(originalAlpha * disabledAlpha));
        }
    }
