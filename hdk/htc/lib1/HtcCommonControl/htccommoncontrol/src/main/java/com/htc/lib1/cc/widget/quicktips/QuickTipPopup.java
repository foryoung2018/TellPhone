...
public class QuickTipPopup extends PopupBubbleWindow{
    ...
    private void init(Context context) {
        TypedArray a = mContext.obtainStyledAttributes(null, R.styleable.PopupBubbleWindow, R.attr.popupBubbleWindowStyle, 0);
        mPopupBackgroundId = a.getResourceId(R.styleable.PopupBubbleWindow_popupBubbleBackground, R.drawable.common_popupmenu);
        final int iconResId = a.getResourceId(R.styleable.PopupBubbleWindow_android_panelFullBackground, R.drawable.icon_btn_cancel_dark_s);
        final int tipsTextId = a.getResourceId(R.styleable
            .PopupBubbleWindow_android_panelTextAppearance,R.style.fixed_darklist_primary_m);
        a.recycle();
        ...
        mMaxWidth = Math.min(screen.widthPixels, screen.heightPixels) - 6 * mMarginM4;
        ...
        mTipText.setTextAppearance(context, tipsTextId);
        ...
        mCloseIconForTextOnly.setImageResource(iconResId);
        mCloseIcon = (ImageView) mContentView.findViewById(R.id.quicktip_close_image);
        mCloseIcon.setImageResource(iconResId);
        ...
        mClearIconWidth = res.getDrawable(iconResId).getIntrinsicWidth();
        int mClearIconHeight =res.getDrawable(iconResId).getIntrinsicHeight();
    ...
    private void initBackgroundColor() {
        Drawable background = mContext.getResources().getDrawable(mPopupBackgroundId);
    ...
    public void setOrientation(byte screen_mode){
        ...
        int right_margin = clear_icon_width == 0 ? mMarginM1 : 2*mMarginM2;
        boolean isLandscape= (screen_mode == SCREEN_MODE_LANDSCAPE || screen_mode == SCREEN_MODE_ILANDSCAPE);
        int ori_img_height = hasImage? mImageSection.getMeasuredHeight()+mMarginM1 :0;
        ...
        if(isLandscape){
                ...
                if(isHeightChange){
                  int textWidth = hasImage ? (ori_height - 2*mMarginM1) : (ori_height - mMarginM1 - right_margin - clear_icon_width);
                  ...
                  ori_height = mTipText.getMeasuredHeight() + ori_img_height+mMarginM2+mMarginM3;
                  widthSpec = MeasureSpec.makeMeasureSpec(ori_height, MeasureSpec.EXACTLY);
                }
            }

        }else{
            int textWidth = mMaxWidth - 2*mMarginM1;
            if(!hasImage && mTipText.getText()!=null && !"".equals(mTipText.getText().toString()))
                textWidth = (int) Math.min(mMaxWidth - mMarginM1 - right_margin - clear_icon_width,
            ...
            int contentWidth = hasImage ? mMaxWidth : (textWidth + mMarginM1 + right_margin + clear_icon_width);
    ...
    private void resetLayoutParam() {
       ...
       int right_margin = clear_icon_width == 0 ? mMarginM1 : 2*mMarginM2;
        ...
        if(hasImage ||  mCloseIconForTextOnly == null || (mCloseIconForTextOnly != null && mCloseIconForTextOnly.getVisibility() != View.VISIBLE))
            ((MarginLayoutParams) text_params).setMargins(mMarginM1,mMarginM3,mMarginM1,mMarginM2);
        else
            ((MarginLayoutParams) text_params).setMargins(mMarginM1,mMarginM3,mMarginM2,mMarginM2);
        ...
        if(!hasImage && mTipText.getText()!=null && !"".equals(mTipText.getText().toString())){
            ...
            if(mParent!=null && mExpandDirection == PopupBubbleWindow.EXPAND_LEFT){
                ...
                if(mContext != null && null != anchor){
                    ...
                    int mPopupWidth = textWidth + mMarginM1 + right_margin + clear_icon_width + mPopupShadowLeft + mPopupShadowRight;
                    ...
                    if (mX < displayFrame.left + (mBubbleLandBodyOffset - mPopupShadowLeft)) {
                        ...
                        textWidth = mPopupWidth - ( mMarginM1 + right_margin + clear_icon_width );
                    }
                }
            }
            // end : check limit
    ...
    protected void resetHeight( WindowManager.LayoutParams params){
        ...
        int textWidth = content_width - 2*mMarginM1;
        if(!hasImage && mTipText.getText()!=null && !"".equals(mTipText.getText().toString())) {
            textWidth = (int) Math.min(content_width - mMarginM1 - right_margin - clear_icon_width,
                    mTipText.getPaint().measureText(mTipText.getText().toString()));
        }
