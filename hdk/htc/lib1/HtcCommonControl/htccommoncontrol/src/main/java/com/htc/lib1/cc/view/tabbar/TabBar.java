...
public class TabBar extends HorizontalScrollView implements HtcViewPager.Decor{
    ...
    private int mDarkTextColor, mLightTextColor;
    private int mDarkTextBackground, mLightTextBackground;
    private int mDarkTabBkgColor, mLightTabBkgColor;
    ...
    public TabBar(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.tabbarStyle);
    }
    ...
    private void setBackgroundMode(int mode) {
        if (mBackgroundMode == mode) return;

        mBackgroundMode = mode;
        init(null, R.attr.tabbarStyle);
    }

    private void init(AttributeSet attrs, int defStyle) {
        final int defStyleAttr = mBackgroundMode == HtcButtonUtil.BACKGROUND_MODE_THEME ? defStyle : 0;
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.HtcTabBar, defStyleAttr, getDefStyleRes());
        mDefBarHeightId = a.getResourceId(R.styleable.HtcTabBar_android_height, R.dimen.tabbar_height);
        mIndicatorHeightId = a.getResourceId(R.styleable.HtcTabBar_android_rowHeight, R.dimen.tab_indicator_height);
        mSelectedIndicatorThickness = getResources().getDimensionPixelOffset(mIndicatorHeightId);
        mTabTextAppearance = a.getResourceId(R.styleable.HtcTabBar_android_itemTextAppearance, R.style.fixed_b_separator_secondary_xs);

        mDarkTextColor = a.getColor(R.styleable.HtcTabBar_android_textColorPrimary, TabBarUtils.color.portriatTextColor(getContext()));
        mLightTextColor = a.getColor(R.styleable.HtcTabBar_android_textColorPrimaryInverse, TabBarUtils.color.landscapeTextColor(getContext()));

        mDarkTabBkgColor = a.getColor(R.styleable.HtcTabBar_android_panelColorForeground, TabBarUtils.color.backgroundColor(getContext()));
        mLightTabBkgColor = a.getColor(R.styleable.HtcTabBar_android_panelColorBackground, TabBarUtils.color.backgroundColor(getContext()));

        mDarkTextBackground = a.getResourceId(R.styleable.HtcTabBar_android_panelBackground, R.drawable.list_selector_dark);
        mLightTextBackground = a.getResourceId(R.styleable.HtcTabBar_android_panelFullBackground, R.drawable.list_selector_light);

        a.recycle();
    }

    private int getDefStyleRes() {
        return R.style.HtcTabBar;
    }
    ...
    private TextView createDefaultTabView(Context context) {
        ...
        textView.setTextAppearance(context, mTabTextAppearance);
    ...
    private void setupBackground() {
        if (isScreenPortriat()) {
            super.setBackgroundDrawable(mBackgroundDrawable != null ? mBackgroundDrawable : new ColorDrawable(mDarkTabBkgColor));
        } else {
            super.setBackgroundDrawable(new ColorDrawable(mLightTabBkgColor));
        }
    }
    ...
    private boolean isScreenPortriat(){
        return getResources().getConfiguration().screenHeightDp > getResources().getConfiguration().screenWidthDp || mUserBarHeight > 0;
    }
    ...
    private void tintTabTextColor(){
        int curTextColor = isScreenPortriat() ? mDarkTextColor : mLightTextColor;
        int otherTextColor = curTextColor & TEXTCOLOR_ALPHA_MASK;

        if(mTabStrip != null){
            for(int i = 0;i < mTabStrip.getChildCount(); i++){
                TextView tv = (TextView)mTabStrip.getChildAt(i);
                tv.setTextColor((null != mViewPager) ? ((i == mViewPager.getCurrentItem()) ?
                    curTextColor : otherTextColor) : otherTextColor);
                Drawable selector = isScreenPortriat() ? getResources().getDrawable(mDarkTextBackground) : getResources().getDrawable(mLightTextBackground);
    ...
    public int getBarHeight() {
        ...
        return getResources().getDimensionPixelOffset(mDefBarHeightId);
    }
