...
package com.htc.lib1.cc.htcListItem;

import android.content.Intent;
import android.util.Log;
import android.view.ViewGroup;

import com.htc.htclistitem.aut.HtcListItemActivity1;
...
public class HtcListItemTestCase extends HtcActivityTestCaseBase {
    ...
    public final void testItem9_round_false() {
        scrollTo(9, false);
        final ViewGroup item = (ViewGroup) getActivity().findViewById(android.R.id.list);
        try {
            runTestOnUiThread(new Runnable() {
                @Override
                public void run() {
                    HtcListItemQuickContactBadge image = (HtcListItemQuickContactBadge) item.findViewById(R.id.photo);
                    image.enableRoundSize(false);
                    image.getBadge().setImageResource(R.drawable.icon_category_photo);
                }
            });
        } catch (Throwable e) {
            e.printStackTrace();
    ...
    public final void testItem12_round_false() {
        scrollTo(12, false);
        final ViewGroup item = (ViewGroup) getActivity().findViewById(android.R.id.list);
        try {
            runTestOnUiThread(new Runnable() {
                @Override
                public void run() {
                    HtcListItemTileImage image = (HtcListItemTileImage) item.findViewById(R.id.photo);
                    image.enableRoundSize(false);
