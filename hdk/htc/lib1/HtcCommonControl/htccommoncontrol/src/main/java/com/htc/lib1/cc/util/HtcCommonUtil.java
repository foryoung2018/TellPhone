...
public class HtcCommonUtil {
     ...
     * light theme
     */
    public static final int THEME_LIGHT =HtcButtonUtil.BACKGROUND_MODE_LIGHT;

    /**
     * dark theme
     */
    public static final int THEME_DARK =HtcButtonUtil.BACKGROUND_MODE_DARK;

    /**
     * car theme
     */
    public static final int THEME_CAR =HtcButtonUtil.BACKGROUND_MODE_AUTOMOTIVEDARK;
    ...
    private static final SparseArray<ChangeObservable> OBSERVABLES = new SparseArray<ChangeObservable>();
    ...
    public static void notifyChange(Context context, int type) {
        ...
        for (int i = 0; i < size; i++) {
            Integer key = OBSERVABLES.keyAt(i);
            if ((key & type) != 0) {
                ChangeObservable observable = OBSERVABLES.get(key);
    ...
    public static void initTheme(ContextThemeWrapper context, int category) {
        initTheme(context, category, null);
    }
    ...
    public static void initTheme(ContextThemeWrapper context, int category, String customPath) {
        ...
        HtcThemeUtils.init(context, category, customPath);
    ...
    public static void initTheme(ContextThemeWrapper context, int category, int userHandle, boolean recreate) {
        ...
        HtcThemeUtils.init(context, category, true, userHandle, recreate);
        TraceCompat.endSection();
    }
    ...
    /**
     * To get the current theme type. Such as {@link HtcCommonUtil#THEME_LIGHT},{@link HtcCommonUtil#THEME_DARK},{@link HtcCommonUtil#THEME_CAR}.
     *
     * @param context The Context the view is running in, through which it can access the current
     *                theme, resources, etc. and MUST be blong to the subclass of ContextThemeWrapper.
    ...
    public static int getThemeType(Context context) {
        ...
        TypedArray a = context.obtainStyledAttributes(null, R.styleable.HtcAnimationButtonMode, R.attr.backgroundMode, 0);
        int mode = a.getInt(R.styleable.HtcAnimationButtonMode_backgroundMode, THEME_LIGHT);
        a.recycle();
        return mode;
    }
