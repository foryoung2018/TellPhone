...
public class Utils {
	...
	// Northern California
	public static final String US_WEST_HOST = "s3-us-west-1.amazonaws.com";
	// public static final String DEFAULT_HOST = "10.9.9.148";

	public static final int SECURE_PORT = 443;
	public static final int INSECURE_PORT = 80;
	// public static final int INSECURE_PORT = 8080;
	...
	static String makeCanonicalString(String method, String bucket, String key,
	 ...
	 */
	static String makeCanonicalString(String method, String bucketName,
			String key, Map pathArgs, Map headers, String expires) {
		...
		return makeCanonicalCommonString(bucketName, key, pathArgs, expires, interestingHeaders, buf);
	}
	
	// begin-refine-duplicate-codes-pitt-20120711
	/**
	 * Retrieve the common parts of makeCanonicalStringForHttpClient() & makeCanonicalString() to reduce duplicate codes
	 */
	private static String makeCanonicalCommonString( String bucketName, String key, 
			Map pathArgs, String expires, SortedMap interestingHeaders, StringBuffer buf) {
		...
		for (Iterator i = interestingHeaders.keySet().iterator(); i.hasNext();) {
			...
			if (headerKey.startsWith(AMAZON_HEADER_PREFIX)) {
				buf.append(headerKey).append(':').append(
						interestingHeaders.get(headerKey));
			} else {
	...
	// end-refine-duplicate-codes-pitt-20120711
	...
	static Map paramsForListOptions(String prefix, String marker,
			Integer maxKeys) {
	...
	public static String convertPathArgsHashToString(Map pathArgs) {
		...
		if (pathArgs != null) {
			for (Iterator argumentIterator = pathArgs.keySet().iterator(); argumentIterator
					.hasNext();) {
	 ...
	 */
	static boolean validateBucketName(String bucketName,
			CallingFormat callingFormat, boolean located) {
	...
	static CallingFormat getCallingFormatForBucket(CallingFormat desiredFormat,
			String bucketName) {
		CallingFormat callingFormat = desiredFormat;
		if (callingFormat == CallingFormat.getSubdomainCallingFormat()
				&& !Utils.isValidSubdomainBucketName(bucketName)) {
	...
	public static String computeMD5Hash(String s) {
		try {
			...
			return Base64.encodeBytes(passDigest);
		} catch (NoSuchAlgorithmException e) {
			e.printStackTrace();
		}
	 ...
	 * @return
	 */
	public static String computeMD5Hash(InputStream is) {
		try {
			MessageDigest digest = java.security.MessageDigest
					.getInstance("MD5");
	...
	public static long getContentLength(String s){
		long length=0;
