...
public class HtcExpandableListView extends ExpandableListView implements Refreshable,
    ...
    @ExportedProperty(category = "CommonControl")
    private int mExpandableItemBackroundColor = Color.WHITE;
    ...
    public HtcExpandableListView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.HtcListView, defStyle, R.style.HtcListView);
        mExpandDividerHeight = a.getDimensionPixelOffset(R.styleable.HtcListView_android_height, res.getDimensionPixelOffset(R.dimen.expand_divider_height));
        mExpandDivider = res.getDrawable(a.getResourceId(R.styleable.HtcListView_android_divider, R.drawable.inset_list_divider));
        mExpandableItemBackroundColor = res.getColor(a.getResourceId(R.styleable.HtcListView_android_itemBackground, R.color.HtcExpandableListView_childItem_backgroundColor));
        a.recycle();

        a = context.obtainStyledAttributes(R.style.HtcListView_Dark, R.styleable.HtcListView);
        mDarkExpandDivider = res.getDrawable(a.getResourceId(R.styleable.HtcListView_android_divider, R.drawable.inset_list_divider_dark));
        a.recycle();
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (isExpandedGroup && i-1 != lastExpandDividerDrawnIndex) {
                ...
                divider.setBounds(0, b - mExpandDividerHeight, getWidth(), b);
                divider.draw(canvas);
            }
    ...
    /**
     * To get child item's background color in current theme.
     * @return child item's background color in current theme.
     */
    public int getExpandableItemBackgroundColor(){
        return mExpandableItemBackroundColor;
    }
}
