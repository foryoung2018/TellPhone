...
public class HtcListItemMessageBody extends FrameLayout implements IHtcListItemTextComponent,
    ...
    private HtcListItemMessageBody(Context context, AttributeSet attrs, int defStyle, int mode) {
        super(context, attrs, defStyle);

        CheckUtil.isContextThemeWrapper(context);
        CheckUtil.isUIThread(context);
        if (mode >= HtcListItem2TextComponent.MODE_WHITE_LIST && mode <= HtcListItem2TextComponent.MODE_DARK_LIST) {
            mMode = mode;
        } else {
            mMode = HtcListItem2TextComponent.MODE_UNKNOWN_LIST;
        }
    ...
    private void setDefaultTextStyle(AttributeSet attrs) {
        ...
        int secondaryTextStyleId = a.getResourceId(R.styleable.HtcListItemAppearance_android_textAppearanceSmall,
                R.style.info_primary_m);
        int stampTextStyleId = a.getResourceId(R.styleable.HtcListItemAppearance_android_textAppearanceInverse,
                R.style.separator_secondary_m);
        int bodyTextStyleId = a.getResourceId(R.styleable.HtcListItemAppearance_android_textAppearanceLarge,
                R.style.list_secondary_m);
        a.recycle();
        setPrimaryTextStyle(primaryTextStyleId);
        setSecondaryTextStyle(secondaryTextStyleId);
        setStampTextStyle(stampTextStyleId);
        setBodyTextStyle(bodyTextStyleId);

        // TODO In dark mode, will get darklist_primary_m , but now need darklist_primary_m_bold
        if((mItemMode == HtcListItem.MODE_DEFAULT || mItemMode == HtcListItem.MODE_KEEP_MEDIUM_HEIGHT)
                && mMode == HtcListItem2TextComponent.MODE_DARK_LIST){
