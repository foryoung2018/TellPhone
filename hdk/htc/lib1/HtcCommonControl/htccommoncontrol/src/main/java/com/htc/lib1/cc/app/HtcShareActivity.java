...
public class HtcShareActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mTheme == FLAG_THEME_DARK) mTheme = HtcCommonUtil.BASELINE;
        int style = intent.getIntExtra(EXTRA_THEME_ID, R.style.HtcDeviceDefault);

        getTheme().applyStyle(style, false);
        HtcCommonUtil.initTheme(this, mTheme);
        ...
        // set content view
        setContentView(R.layout.activity_htcshareactivity);
        TypedArray a = getTheme().obtainStyledAttributes(null, R.styleable.HtcShareVia, R.attr.htcShareViaStyle, R.style.HtcShareVia_Light);
        int headerColor = a.getColor(R.styleable.HtcShareVia_android_headerBackground, HtcCommonUtil.getCommonThemeColor(this, R.styleable.ThemeColor_multiply_color));
        int itemTextAppearance = a.getResourceId(R.styleable.HtcShareVia_android_itemTextAppearance, R.style.list_body_primary_xs);
        int gridBackgroundColor = a.getColor(R.styleable.HtcShareVia_android_panelBackground, getResources().getColor(R.color.ap_background_color));
        Drawable gridListSelector = a.getDrawable(R.styleable.HtcShareVia_android_listSelector);
        a.recycle();
        ...
        title.setBackgroundColor(headerColor);
        ...
        if (DEBUG) Log.d(TAG, "onCreate: color=" + Integer.toHexString(headerColor));
        ...
        mAdapter = new HtcShareAdapter(mIntents, mAllowed, mBlocked, this, itemTextAppearance, mQueryByPackage);
        ...
        gridView.setBackgroundColor(gridBackgroundColor);
        gridView.setSelector(gridListSelector);
    ...
    private int getCollapsedHeightOfHtcShareActivity() {
        ...
        if (null != config && config.orientation == Configuration.ORIENTATION_PORTRAIT) {
...
//    }
    ...
    public static void startActivityForResult(Intent intent, int requestCode, Activity activity) {
        if (intent != null && !intent.hasExtra(HtcShareActivity.EXTRA_THEME_ID)) {
            int themeId = resolveHtcShareActivityTheme(activity);
            intent.putExtra(HtcShareActivity.EXTRA_THEME_ID, themeId);
        }
     ...
     * @hide
     */
    public static int resolveHtcShareActivityTheme(Context context) {
        if (context == null) return R.style.HtcDeviceDefault;
        TypedValue out = new TypedValue();
        context.getTheme().resolveAttribute(R.attr.htcShareActivityTheme, out, true);
        if (out.resourceId > 0) return out.resourceId;
        return R.style.HtcDeviceDefault;
    }
}
