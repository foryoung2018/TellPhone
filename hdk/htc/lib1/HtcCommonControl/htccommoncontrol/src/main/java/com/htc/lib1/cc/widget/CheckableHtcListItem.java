...
public class CheckableHtcListItem extends HtcListItem implements Checkable {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        a.recycle();
        a = context.obtainStyledAttributes(attrs, R.styleable.HtcListItemAppearance, R.attr.htcListItemStyle, R.style.htcListItem);
        mStyleId = a.getResourceId(R.styleable.HtcListItemAppearance_android_textColorSecondary, 0);
        a.recycle();
    ...
    private void setupAutomotiveMode() {
        ...
        if (text instanceof HtcListItemSingleText) {
            ((HtcListItemSingleText) text).setUseFontSizeInStyle(true);
            ((HtcListItemSingleText) text).setTextStyle(mStyleId);
        }
    }
