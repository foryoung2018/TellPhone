...
/*package*/ final class HtcThemeUtils {
    ...
    private static class ThemePathObserver extends ContentObserver {
        ...
        public void onChange (boolean selfChange, Uri uri) {
            ThemeLogUtil.LogD("[ThemePathObserver.onChange] selfChange="+selfChange+", uri="+uri);
    ...
    /*package*/ static boolean setHtcThemePackage(Context context, int type, String themePackage) {
        ...
        if (context!=null && themeType!=null && themePackage!=null) {
            ...
            if (!themePackage.equals(originalPackage)) {
                ...
                ThemeLogUtil.LogD("Set thm pkg from \'" + originalPackage + "\' to \'" + themePackage + "\' for type " + type);
            }
        }
    ...
    private static class ThemeKeyObserverForUsers extends ContentObserver {
        ...
        public void onChange (boolean selfChange, Uri uri) {
            String themeType = getThemeTypeFromUri(uri);
            if (themeType == null) return;
            ThemeLogUtil.LogD("[ThemeKeyObserverForUsers][onChange] type=" + themeType);
    ...
    /* package */static Resources getResources(Context context, String apkName, String apkPath, String optionalPath) {
        ...
        ThemeLogUtil.LogD("context=" + context + ", target=" + target);
        ThemeLogUtil.LogD("optionalPath=" + optionalPath);
        try {
            ...
            if((cookieObj instanceof Integer) && (((Integer)cookieObj).intValue()==0)) {
                ...
                ThemeLogUtil.LogD("AssetMaanger addAssetPath " + apkName + " fail!");

                if (TextUtils.isEmpty(optionalPath)) return res;

                target = optionalPath + apkName + RES_FILE_EXTENSION;
                ThemeLogUtil.LogD("context=" + context + ", optional=" + target);
                cookieObj = assetMag_addAssetPathMtd.invoke(assetMag, new Object[] { target });
                if ((cookieObj instanceof Integer) && (((Integer) cookieObj).intValue() == 0)) {
                    ThemeLogUtil.LogD("AssetMaanger addOptionalPath " + apkName + " fail!");
                    return res;
                }
            }
    ...
    /* package */static void init(Context context, int category, String customPath) {
        mCustomAPKPath = customPath;
        init(context, category, false, 0, false);
    }
    ...
    private static class CommonThemeResources {
        ...
        private static Drawable getTexture(Context context, int index, String customPath) {
            ...
            if (ret == null) {
                String targetPath = (customPath == null ? getCurrentThemePath(mByUser, mUserHandle) : customPath);
    ...
    private static String getThemeTypeFromUri(Uri uri) {
        if (uri == null) {
            ThemeLogUtil.LogD("Theme type uri is null", new Exception());
            return null;
        }

        if (HtcBuildFlag.Htc_DEBUG_flag) {
            if (uri.getPathSegments().size() < 2) ThemeLogUtil.LogD("Invalid theme type uri = " + uri, new Exception());
        }

        String themeType = uri.getLastPathSegment();
        if (TextUtils.isEmpty(themeType)) {
            if (HtcBuildFlag.Htc_DEBUG_flag) ThemeLogUtil.LogD("Invalid theme type = " + themeType, new Exception());
            return null;
        }
    ...
    private static String getStringForUser(Context context, String themeType, int userHandle) {
        ThemeLogUtil.LogD("[getStringForUser] themeType=" + themeType + ", userHandle=" + userHandle);
    ...
    private static boolean setCurrentThemePackageInternal(Context context, int type, String themePackage, boolean byUser, int userHandle) {
        ...
        if (context!=null && themeType!=null && themePackage!=null) {
            ...
            if (!themePackage.equals(originalPackage)) {
                ...
                ThemeLogUtil.LogD("Set ["+themeType+"] from ["+originalPackage+"] to ["+themePackage+"]", new Throwable());
            }
        }
    ...
    private static void putStringForUser(Context context, String key, String value, int userHandle) {
        ThemeLogUtil.LogD("[putStringForUser] key=" + key + ", value=" + value + ", userHandle=" + userHandle);
    ...
    private static Resources getResourcesInternal(Context context, String apkName, boolean byUser, int userHandle) {
        ...
        return getResources(context, apkName, getCurrentThemePathInternal(byUser, userHandle), mCustomAPKPath);
    }

    /* package */static void init(Context context, int category, boolean byUser, int userHandle, boolean recreate) {
        if (recreate) {
            setIfNeedReloadPath(true);
            CommonCategoryResources.setRecreateFlag(true);
            CommonThemeResources.setRecreateFlag(true);
        }
        initInternal(context, category, byUser, userHandle);
    }

    private static void initInternal(Context context, int category, boolean byUser, int userHandle) {
        ThemeLogUtil.LogD("context=" + context + ", category=" + category + ", byUser=" + byUser + ", userHandle=" + userHandle);
        if (context == null) throw new IllegalArgumentException(ARGS_ERR_MSG);
