...
public class HtcAutoCompleteTextView extends AutoCompleteTextView {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.HtcAutoCompleteTextView, defStyle, R.style.htcAutoCompleteTextViewStyleDefault);
        mRestDrawableId = a.getResourceId(R.styleable.HtcAutoCompleteTextView_android_panelBackground, R.drawable.common_inputfield_rest);
        mPressedDrawableId = a.getResourceId(R.styleable.HtcAutoCompleteTextView_android_panelFullBackground, R.drawable.common_inputfield_pressed);
        mMinHeight = res.getDrawable(mRestDrawableId).getIntrinsicHeight();

        mTextStyleId=a.getResourceId(R.styleable.HtcAutoCompleteTextView_android_textAppearanceLarge, R.style.input_default_m);
        mDisabledAlpha = a.getFloat(R.styleable.HtcAutoCompleteTextView_android_disabledAlpha, HtcInputFieldUtil.DISABLED_ALPHA_LIGHT);
        int colorEmpty=a.getColor(R.styleable.HtcAutoCompleteTextView_android_textColorPrimary, res.getColor(R.color.HtcAutoCompleteTextView_color_empty));
    ...
    private Drawable getModeBackground(int mode) {
        ...
        switch (mode) {
            ...
            case MODE_BRIGHT_BACKGROUND:
                cs = mLight;
                pressedDraw = HtcEditText.getRealBackground(cs, res.getDrawable(mPressedDrawableId == 0 ? R.drawable.common_inputfield_pressed : mPressedDrawableId));
                restDraw = HtcEditText.getRealBackground(cs, res.getDrawable(mRestDrawableId == 0 ? R.drawable.common_inputfield_rest : mRestDrawableId));
                break;
            default:
                break;
        }
    ...
    private int getMinDropDownWidth() {
        boolean isAutomotive = mSupportMode == MODE_AUTOMOTIVE;
        if (mSupportMode == HtcButtonUtil.BACKGROUND_MODE_THEME) {
            isAutomotive = HtcCommonUtil.getThemeType(getContext()) == HtcCommonUtil.THEME_CAR;
        }
        return Math.min(getResources().getDisplayMetrics().widthPixels, getResources().getDisplayMetrics().heightPixels)
                - ActionBarUtil.getActionBarBackupViewWidth(getContext(), isAutomotive) - mMeasureSpecM2;
    }
